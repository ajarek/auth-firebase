  ...
  getUserData(token).then((res) => {
    ...
    if (res.error) {
      ...
      document.querySelector(".data").innerHTML = `
            ...
            last login at:${new Date(
              Number(res.users[0].lastLoginAt)
            ).toLocaleString("pl")}<br>
            created at:${new Date(
              Number(res.users[0].createdAt)
            ).toLocaleString("pl")}<br>`;
    }
  });
};
...
const eventUserData = () => {
  document
    .querySelector("#btnUserData")
    .addEventListener("click", readUserData);
};
//update User
...
const eventSubmitUpdateUserData = () => {
  document
    .querySelector("#formUpdateUser")
    .addEventListener("submit", submitUpdateUserData);
};
